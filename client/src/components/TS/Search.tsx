...
const Search: FC<SearchProps> = ({ setInput }) => {
    ...
    async function getNames(input: string) {
        const response = await fetch(`https://registry.npmjs.org/-/v1/search?text=${input}&size=6`)
    ...
    return (
        <form onSubmit={submitHandler} className='w-full'>
            <div className="flex gap-2 w-full">
                ...
                <button onClick={submitHandler} className="btn tracking-wide inline-block px-6 py-2 border-2 border-blue-600 text-blue-600 font-medium text-s leading-tight rounded focus:outline-none focus:ring-0 transition duration-150 ease-in-out hover:bg-blue-600 hover:text-white" type="button" id="button-addon3">Search</button>
            </div>
            <div className="flex gap-2 w-full">
                <ul className='w-full'>
